  ...
  var ProcessForm = function (parameters) {
        ...
        this.init = function () {
            ...
            if (this.existenceUploadsFile) { // добавление новых элементов input с type="file" и изменение существующих
              ...
              if (this.hideForm) {
                var self = this;
                $(submitForm).parent().next('.card-footer').find('.show-form').click(function (e) {
                  e.preventDefault();
                  $(this).closest('.card-footer').addClass('d-none');
    ...
    ProcessForm.prototype.changeStateSubmit = function (state) {
      var submitForm = document.getElementById(this.idForm);
      // $(submitForm).find('[type="submit"]').map('disabled', state);
              ...
              data = JSON.parse(data);
                ...
                if (data.result === "success") {
                  ...
                  if (_this.hideForm) {
                    ...
                    submitForm.parent().next('.card-footer').removeClass('d-none');
                  }
                } else {
